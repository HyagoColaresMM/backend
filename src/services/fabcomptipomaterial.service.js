...
const Listar = async (descricao) => {
    return new Promise((resolve, reject) => {
        let ssql = 'SELECT ID, DESCRICAO FROM FABCOMP_TIPOMATERIAL WHERE ID > 0 ';
        const params = [];
...
const Inserir = (id, descricao, callback) => {
    ...
    let ssql =
        "INSERT INTO FABCOMP_TIPOMATERIAL(ID, DESCRICAO) VALUES(?, ?) RETURNING ID";
...
const Editar = (id, descricao) => {
    return new Promise((resolve, reject) => {
        let ssql = 'UPDATE FABCOMP_TIPOMATERIAL SET ';
        const params = [];
