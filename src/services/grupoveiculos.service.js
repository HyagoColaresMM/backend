...
const Listar = (descricao, limit, skip) => {
    return new Promise((resolve, reject) => {
        ...
        ssql += ' ID, DESCRICAO, CREATED_AT, UPDATED_AT FROM GRUPO_VEICULOS WHERE DELETED_AT IS NULL ';
        let params = [];
...
const Inserir = (descricao, callback) => {
    let ssqlMaxId = "SELECT MAX(ID) AS MAX_ID FROM GRUPO_VEICULOS";
    ...
    firebird.attach(dbOptions, (err, db) => {
        ...
        db.query(ssqlMaxId, [], (err, result) => {
            ...
            let ssqlInsert = "INSERT INTO GRUPO_VEICULOS(ID, DESCRICAO, CREATED_AT) VALUES(?, ?, CURRENT_TIMESTAMP) RETURNING ID";
...
const Editar = (id, descricao) => {
    return new Promise((resolve, reject) => {
        let ssql = 'UPDATE GRUPO_VEICULOS SET UPDATED_AT = CURRENT_TIMESTAMP, ';
        const params = [];
...
const Deletar = (id) => {
    return new Promise((resolve, reject) => {
        let ssql = 'UPDATE GRUPO_VEICULOS SET DELETED_AT = CURRENT_TIMESTAMP, ';
        const params = [];
...
const Deletar2 = (id, callback) => {
    let params = [id]
    let ssql = "DELETE FROM GRUPO_VEICULOS WHERE ID = ? "; //AND DELETED_AT = ''
