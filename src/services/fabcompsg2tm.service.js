...
const Listar = async (descricao) => {
    return new Promise((resolve, reject) => {
        let ssql = 'SELECT ID, DESCRICAO FROM FABCOMP_SG2_TM WHERE ID > 0 ';
        const params = [];
...
const Inserir = (id, descricao, callback) => {
    ...
    let ssql =
        "INSERT INTO FABCOMP_SG2_TM(ID, DESCRICAO) VALUES(?, ?) RETURNING ID";
...
const Editar = (id, descricao) => {
    return new Promise((resolve, reject) => {
        let ssql = 'UPDATE FABCOMP_SG2_TM SET ';
        const params = [];
