...
const Listar = (descricao, limit, skip) => {
    return new Promise((resolve, reject) => {
        ...
        ssql += ' ID, DESCRICAO FROM FOLHA_SETORES WHERE DELETED_AT IS NULL ';
        let params = [];
...
const Inserir = (descricao, callback) => {
    let ssqlMaxId = "SELECT MAX(ID) AS MAX_ID FROM FOLHA_SETORES";
    ...
    firebird.attach(dbOptions, (err, db) => {
        ...
        db.query(ssqlMaxId, [], (err, result) => {
            ...
            let ssqlInsert = "INSERT INTO FOLHA_SETORES(ID, DESCRICAO, CREATED_AT) VALUES(?, ?, CURRENT_TIMESTAMP) RETURNING ID";
...
const Editar = (id, descricao) => {
    return new Promise((resolve, reject) => {
        let ssql = 'UPDATE FOLHA_SETORES SET UPDATED_AT = CURRENT_TIMESTAMP, ';
        const params = [];
...
const Deletar = (id) => {
    return new Promise((resolve, reject) => {
        let ssql = 'UPDATE FOLHA_SETORES SET DELETED_AT = CURRENT_TIMESTAMP, ';
        const params = [];
...
const Deletar2 = (id, callback) => {
    let params = [id]
    let ssql = "DELETE FROM FOLHA_SETORES WHERE ID = ? "; //AND DELETED_AT = ''
