...
const Listar = (descricao) => {
    return new Promise((resolve, reject) => {
        let ssql = 'SELECT ID, DESCRICAO, CREATED_AT, UPDATED_AT FROM FOLHA_RUBRICAS WHERE DELETED_AT IS NULL ';
        let params = [];
...
const Inserir = (descricao, callback) => {
    ...
    firebird.attach(dbOptions, (err, db) => {
        ...
        db.query(ssqlMaxId, [], (err, result) => {
            ...
            let ssqlInsert = "INSERT INTO FOLHA_RUBRICAS(ID, DESCRICAO, CREATED_AT) VALUES(?, ?, CURRENT_TIMESTAMP) RETURNING ID";
...
const Editar = (id, descricao) => {
    return new Promise((resolve, reject) => {
        let ssql = 'UPDATE FOLHA_RUBRICAS SET UPDATED_AT = CURRENT_TIMESTAMP, ';
        const params = [];
...
const Deletar = (id) => {
    return new Promise((resolve, reject) => {
        let ssql = 'UPDATE FOLHA_RUBRICAS SET DELETED_AT = CURRENT_TIMESTAMP, ';
        const params = [];
...
const Deletar2 = (id, callback) => {
    let params = [id]
    let ssql = "DELETE FROM FOLHA_RUBRICAS WHERE ID = ? "; //AND DELETED_AT = ''
