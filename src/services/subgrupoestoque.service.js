...
const Listar = (descricao) => {
    return new Promise((resolve, reject) => {
        let ssql = 'SELECT ID, DESCRICAO, CREATED_AT, UPDATED_AT FROM SUBGRUPO_ESTOQUE WHERE DELETED_AT IS NULL ';
        const params = [];
...
const Deletar = (id) => {
    return new Promise((resolve, reject) => {
        let ssql = 'UPDATE SUBGRUPO_ESTOQUE SET DELETED_AT = CURRENT_TIMESTAMP, ';
        const params = [];
...
const Deletar2 = (id, callback) => {
    let params = [id]
    let ssql = "DELETE FROM SUBGRUPO_ESTOQUE WHERE ID = ? "; //AND DELETED_AT = ''
