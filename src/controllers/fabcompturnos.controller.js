...
const Editar = (req, res) => {
    ...
    ServiceFabcompTurnos.Editar(id, turno, descricao, horaInicialTurno, horaFinalTurno, horaProgramada, tempoDeAlmoco,)
        .then(() => {
            res.status(200).json({ message: 'Turnos atualizado com sucesso' });
        })
        .catch((err) => {
    ...
    ServiceFabcompTurnos.Deletar(id, (err, result) => {
        if (err) {
            ...
            if (result && result.length === 0) {
                res.status(404).json({ message: "Turnos não encontrado." });
            } else {
                res.status(200).json({ message: "Turnos excluído com sucesso." });
            }
        }
    });
};
